...
route.post('/', authenticate.user_auth, async (req: ExtendRequest, res: Response): Promise<void> => {
    ...
    try {
        ...
        const tranAllPosts = await page_managerUserController.selectPosts(selectedTargetId);
...
route.post(
    ...
    async (req: ExtendRequest, res: Response): Promise<void> => {
        try {
            ...
            const result = await page_managerUserController.updateUser(id, name);
...
route.post('/upload/avatar', upload.single('image'), handleUpload, authenticate.user_auth, async (req: ExtendRequest, res: Response): Promise<void> => {
    try {
        const id = req.admin?.id;
        ...
        const result = await page_managerUserController.updateAvatarUser(id, imagePath);
        if (result !== 0) {
            res.redirect('/main');
        } else {
            res.status(500).json({ message: 'Update error' });
...
route.post('/upload/thumbnail', upload.single('image'), handleUpload, authenticate.user_auth, async (req: ExtendRequest, res: Response): Promise<void> => {
    try {
        const id = req.admin?.id;
        const imagePath = `/pictures/${req.file?.filename}`;
        if (!id) {
        ...
        const result = await page_managerUserController.updateThumbnailUser(id, imagePath);
        if (result !== 0) {
            res.redirect('/main');
        } else {
            res.status(500).json({ message: 'Update error' });
...
route.post('/upload/informationuser', authenticate.user_auth, async (req: ExtendRequest, res: Response): Promise<void> => {
    try {
        ...
        const result = await page_managerUserController.updateInformationsUser(id, name, hastag, hometown, school);
        if (result !== 0) {
            res.redirect('/main');
        } else {
            res.status(500).json({ message: 'Update error' });
