...
route.get('/', authenticate.user_auth, async (req: any, res: Response): Promise<any> => {
    try {
        ...
        console.log(essays_tranforme.length);
...
//     try {
//         const idGroup = req.session.currentGroupId;
//         let groups = await model_group.findAll({ where: { id: idGroup } });
//         let group = groups.map((b: any) => {
//             b.toJSON();
//         })
//         let essays = await model_essays.select({ group_id: idGroup, scope: 'group' })
//         let essay = essays.map((b: any) => {
//             b.toJSON();
//         })
//         res.render('contens/page_manager/essay', { group: group, essay: essay })
